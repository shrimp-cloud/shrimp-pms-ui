...
<script setup name="MerchantCouponPage">
import {
  merchantCouponPage,
  merchantCouponRemove,
  merchantCouponStatusOn,
  merchantCouponStatusOff,
} from '@/api/merchantCoupon';
...
function getList() {
  loading.value = true;
  merchantCouponPage(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
    const data = res.data;
...
function handlePublish(row) {
  ...
  if (row.couponStatus === 'ISSUED') {
    merchantCouponStatusOn({id: row.id}).then((res) => {
      getList();
    }).finally(() => {
      loading.value = false;
    });
  } else {
    merchantCouponStatusOff({id: row.id}).then((res) => {
      getList();
    }).finally(() => {
      loading.value = false;
    });
...
function handleDelete(row) {
  proxy.$modal.confirm('是否确认删除为"' + row.title + '"的数据项？').then(() => {
    merchantCouponRemove({id: row.id}).then(res => {
      if (res.code === 1) {
        getList();
