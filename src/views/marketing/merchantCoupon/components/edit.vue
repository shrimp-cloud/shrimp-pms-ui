      ...
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          ...
          <el-form-item label="LOGO" prop="logo">
            <sh-image v-model="form.logo" placeholder="请选择图标"/>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
...
<script setup name="MerchantCouponEdit">
import {
  merchantCouponInfo,
  merchantCouponSave,
...
function handleEdit(row) {
  ...
  if (!row || !row.id) {
    ...
    merchantCouponInfo({id:row.id}).then(res => {
      form.value = res.data;
      open.value = true;
      title.value = "修改";
    });
...
function submitForm() {
  proxy.$refs["editRef"].validate(valid => {
    if (valid) {
      ...
      merchantCouponSave(form.value).then(res => {
